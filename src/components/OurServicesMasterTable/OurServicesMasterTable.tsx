...
const OurServicesMasterTable: React.FC<Props> = ({
  ...
  console.log("MASTER SUB SERVICESS", masterSubServices.data);
  ...
  //   []
  // );
  const searchColumns = useMemo(() => ["name", "description"], []); // Changed this line
      ...
      masterSubServices?.data
        ...
        .map((value: any, index: number) => ({
          ...
          originalSizeImage: value.image
            ? `https://rabbitvpn.sgp1.digitaloceanspaces.com/${value.image.url}`
            : "/logo512.png",
          originalIconSizeImage: value.icon_image
            ? `https://rabbitvpn.sgp1.digitaloceanspaces.com/${value.icon_image.url}`
            : "/logo512.png",
  ...
  const columns = [
    ...
    {
      title: "ICON_IMAGE",
      dataIndex: "originalIconSizeImage",
      width: 130,
      render: (r: any) => (
        <img
          style={{ width: "70px", height: "70px" }}
