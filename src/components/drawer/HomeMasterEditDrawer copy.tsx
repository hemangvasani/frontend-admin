...
const homeMasterValidationSchema = Yup.object().shape({
  image: Yup.mixed()
    .test("fileSize", "File too large", (value) => {
      if (!value) return true; // Allow no file upload
      return value instanceof File && value.size <= 20 * 1024 * 1024; // 20MB max
    })
  ...
  tags: Yup.string()
    .required("Tags are required") // Ensures the field is not empty
    .test("multiple-tags", "Invalid tags", function (value) {
      if (!value) return true; // If no value, required validation will handle it
...
const HomeMasterEditDrawer: React.FC<Props> = ({
  ...
  const onSubmit = (values: Record<string, any>) => {
    ...
    let payload: Record<string, any> = {
      ...
      // key_features: [{ heading: values.title, paragraph: values.keydesc }],
  ...
  // Side effect for handling success
  ...
  return (
    <>
                    ...
                    <DrawerBody>
                      ...
                      <FormControl pb={3}>
                        <FormField
                          ...
                          type="text"
                          // error={
                          //   touched.tags ? (errors.tags as string) : undefined
                          // }
                          error={undefined}
                        />
                      ...
                      {/* <FormControl pb={3}>
                        <FormField
                          ...
                          error={
                            touched.keydesc
                              ? (errors.keydesc as string)
                              : undefined
                          }
                        />
                        {touched.keydesc && errors.keydesc && (
                          <Text color="red.500" fontSize="sm">
                            {errors.keydesc}
                          </Text>
                        )}
      ...
      {/* Modal for adding new home service */}
      {isNewSubServiceOpen && (
        <AddNewHomeModal
          isOpen={isNewSubServiceOpen}
          onClose={onNewSubServiceClose}
